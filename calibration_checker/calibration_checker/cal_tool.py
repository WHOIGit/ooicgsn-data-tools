...
Tabs:
  ...
  4. Instrument Compare — compare an instrument's native calibration file
                          (e.g. CTDMO .cal) against an independently
                          generated CI calibration CSV, to catch
...
try:
    ...
    from .dofstk import parse_dofstk
    from .parad import pdf_parser as parad_pdf_parser
    from .nutnr import cal_parser as nutnr_cal_parser
    from .common import compare_source_to_ci, summarize as ci_summarize
except ImportError:
    # Fallback for running this file directly (`python cal_tool.py`) with no
    # enclosing package on sys.path.
...
# Registry of supported instrument families for the "Instrument Compare" tab.
# Each entry maps a display label to a parser function that takes a source
# calibration file path and returns a DataFrame with columns
# serial, name, value[, source_file] — using the same coefficient-naming
# convention as that instrument's CI calibration CSV. Add new instrument
# families here as their parsers are built (FLORT, CDOM/CTDBP, ...).
# Registry of supported instrument families for the "Instrument Compare" tab.
# Each entry has:
#   "sources": a list of one or more input-file slots, each a dict with
#       "key" (unique within the instrument), "label" (shown in the UI),
#       "parse" (filepath -> DataFrame with serial/name/value[/source_file],
#       using the same coefficient-naming convention as the CI CSV), and
#       "file_types" (for the file-open dialog).
#   "combine": function(dict[key -> DataFrame]) -> one combined DataFrame,
#       once every slot has been loaded. Most instruments have a single
#       source and just return it unchanged; DOFSTK combines two.
#   "metadata": optional function(dict[key -> filepath]) -> dict, shown
#       nowhere yet but available for future use / debugging.
# Add new instrument families here — no other Tab 4 code needs to change.
INSTRUMENT_PARSERS = {
    "CTDMO (SBE 37-IM/IMP) — .cal file": {
        "sources": [
            {"key": "cal", "label": "Native calibration file",
             "parse": ctdmo_cal_parser.parse_cal_file,
             "file_types": [("Cal files", "*.cal *.CAL *.txt *.TXT"), ("All files", "*.*")]},
        ],
        "combine": lambda dfs: dfs["cal"],
    },
    "DOSTA (Aanderaa Optode 4831/4330) — cal certificate PDF": {
        "sources": [
            {"key": "pdf", "label": "Native calibration file",
             "parse": dosta_pdf_parser.parse_dosta_pdf,
             "file_types": [("PDF files", "*.pdf *.PDF"), ("All files", "*.*")]},
        ],
        "combine": lambda dfs: dfs["pdf"],
    },
    "FLORT (WET Labs ECO Triplet, e.g. BBFL2W) — .dev file": {
        "sources": [
            {"key": "dev", "label": "Native calibration file",
             "parse": flort_dev_parser.parse_dev_file,
             "file_types": [("Dev files", "*.dev *.DEV"), ("All files", "*.*")]},
        ],
        "combine": lambda dfs: dfs["dev"],
    },
    "OPTAA (WET Labs AC-S) — .dev file": {
        "sources": [
            {"key": "dev", "label": "Native calibration file",
             "parse": optaa_dev_parser.parse_dev_file,
             "file_types": [("Dev files", "*.dev *.DEV"), ("All files", "*.*")]},
        ],
        "combine": lambda dfs: dfs["dev"],
    },
    "DOFSTK (SBE 43F fast-response oxygen) — .cal file + Soc-adjusted PDF": {
        "sources": [
            {"key": "cal", "label": "Cal file (.cal)",
             "parse": dofstk_cal_parser.parse_cal_file,
             "file_types": [("Cal files", "*.cal *.CAL *.txt *.TXT"), ("All files", "*.*")]},
            {"key": "pdf", "label": "Soc-adjusted certificate (PDF)",
             "parse": dofstk_soc_pdf_parser.parse_soc_pdf,
             "file_types": [("PDF files", "*.pdf *.PDF"), ("All files", "*.*")]},
        ],
        "combine": lambda dfs: pd.concat([dfs["cal"], dfs["pdf"]], ignore_index=True),
    },
    "PARAD-K (Biospherical QSP-2200 PAR sensor) — cal certificate PDF": {
        "sources": [
            {"key": "pdf", "label": "Native calibration file",
             "parse": parad_pdf_parser.parse_parad_pdf,
             "file_types": [("PDF files", "*.pdf *.PDF"), ("All files", "*.*")]},
        ],
        "combine": lambda dfs: dfs["pdf"],
    },
    "NUTNR-B (Satlantic/Sea-Bird SUNA V2 nitrate analyzer) — .CAL file": {
        "sources": [
            {"key": "cal", "label": "Native calibration file",
             "parse": nutnr_cal_parser.parse_cal_file,
             "file_types": [("Cal files", "*.CAL *.cal *.txt *.TXT"), ("All files", "*.*")]},
        ],
        "combine": lambda dfs: dfs["cal"],
    },
...
def fmtv_compact(v, max_items: int = 6) -> str:
    """
    Like fmtv(), but summarises long lists and matrices instead of
    spelling out every element — an 85x36 temperature-correction matrix
    (OPTAA) would otherwise print 3,060 numbers into one table cell.
    """
    if isinstance(v, (list, tuple)):
        ...
        if len(v) > max_items:
            head = ", ".join(fmtv(x) for x in v[:3])
            return f"[{head}, ... ] ({len(v)} values)"
        return fmtv(v)
    return fmtv(v)
...
class CalTool(tk.Tk):
    ...
    def __init__(self):
        ...
        self._instr_source_dfs: dict[str, pd.DataFrame] = {}   # Tab 4: slot key -> parsed df
        self._instr_ci_path: str | None = None                  # Tab 4: CI csv path
        self._instr_compare_df: pd.DataFrame = pd.DataFrame()
    ...
    def _load_cal_pdf(self):
        ...
        if tess:
            cdom_pdf_parser.TESSERACT_CMD = tess
    ...
    # =========================================================================
    # Tab 4 · Instrument Compare (native cal file vs. CI calibration CSV)
    # =========================================================================

    def _build_tab4(self):
        tab = frm(self.nb, bg=C["bg"])
        self.nb.add(tab, text="  Instrument Compare  ")

        # ── left panel: instrument + file pickers ──────────────────────────
        left = frm(tab, bg=C["bg"], width=340)
        left.pack(side="left", fill="y", padx=(14, 0), pady=14)
        left.pack_propagate(False)

        lbl(left, "Instrument Family", bold=True, bg=C["bg"]).pack(anchor="w", pady=(0, 6))
        self._instr_var = tk.StringVar(value=next(iter(INSTRUMENT_PARSERS)))
        opt = tk.OptionMenu(left, self._instr_var, *INSTRUMENT_PARSERS.keys(),
                             command=lambda _choice: self._on_instrument_change())
        opt.config(bg=C["border"], fg=C["text"], activebackground=C["sel"],
                   relief="flat", font=FB, highlightthickness=0)
        opt["menu"].config(bg=C["panel"], fg=C["text"])
        opt.pack(anchor="w", fill="x", pady=(0, 14))

        lbl(left, "1 · Source file(s)", bold=True, bg=C["bg"]).pack(anchor="w", pady=(0, 4))
        self._sources_frame = frm(left, bg=C["bg"])
        self._sources_frame.pack(anchor="w", fill="x", pady=(0, 10))
        self._source_status_vars: dict[str, tk.StringVar] = {}

        lbl(left, "2 · Independently generated CI CSV", bold=True, bg=C["bg"]).pack(anchor="w", pady=(0, 4))
        dark_btn(left, "＋ Load CI CSV", self._instr_load_ci,
                 accent=True).pack(anchor="w", pady=(0, 4))
        self._instr_ci_var = tk.StringVar(value="No CSV loaded.")
        lbl(left, textvariable=self._instr_ci_var, muted=True,
            bg=C["bg"], wraplength=310, justify="left").pack(anchor="w", pady=(0, 14))

        dark_btn(left, "↻ Compare", self._instr_compare
                 ).pack(anchor="w", pady=(6, 4))
        dark_btn(left, "⤓ Export Report CSV", self._instr_export
                 ).pack(anchor="w")

        self._instr_summary_var = tk.StringVar(value="")
        lbl(left, textvariable=self._instr_summary_var, bold=True,
            bg=C["bg"], wraplength=310, justify="left").pack(anchor="w", pady=(14, 0))

        # ── right panel: results ────────────────────────────────────────────
        right = frm(tab, bg=C["bg"])
        right.pack(side="left", fill="both", expand=True, padx=14, pady=14)

        lbl(right, "Comparison Results", bold=True, bg=C["bg"]).pack(anchor="w", pady=(0, 4))

        rfl, self.instr_tree = dark_treeview(right, [
            ("st",     "",             28),
            ("serial", "Serial",      100),
            ("name",   "Coefficient", 130),
            ("src",    "Source Value", 170),
            ("ci",     "CI CSV Value", 170),
            ("delta",  "Δ %",         100),
            ("note",   "Status",      170),
        ], height=28)
        rfl.pack(fill="both", expand=True)
        self.instr_tree.tag_configure("ok",      background=C["row_ok"],
                                                  foreground=C["green"])
        self.instr_tree.tag_configure("bad",     background=C["row_bad"],
                                                  foreground=C["orange"])
        self.instr_tree.tag_configure("missing", foreground=C["muted"])

        self._rebuild_source_widgets()

    # ── Tab 4 callbacks ───────────────────────────────────────────────────────

    def _current_instrument(self) -> dict:
        return INSTRUMENT_PARSERS[self._instr_var.get()]

    def _on_instrument_change(self):
        """Switching instruments invalidates any previously loaded source files."""
        self._instr_source_dfs = {}
        self._rebuild_source_widgets()
        for item in self.instr_tree.get_children():
            self.instr_tree.delete(item)
        self._instr_summary_var.set("")

    def _rebuild_source_widgets(self):
        """(Re)build one 'load' button + status label per source slot for the
        currently selected instrument. Called on init and whenever the
        instrument dropdown changes, since different instruments need a
        different number of input files (e.g. DOFSTK needs two)."""
        for child in self._sources_frame.winfo_children():
            child.destroy()
        self._source_status_vars = {}

        spec = self._current_instrument()
        for i, slot in enumerate(spec["sources"], start=1):
            label = slot["label"] if len(spec["sources"]) == 1 else f"{i} · {slot['label']}"
            lbl(self._sources_frame, label, bg=C["bg"]).pack(anchor="w", pady=(0, 2))
            dark_btn(self._sources_frame, "＋ Load",
                     lambda key=slot["key"]: self._instr_load_source_slot(key),
                     accent=True).pack(anchor="w", pady=(0, 2))
            status_var = tk.StringVar(value="No file loaded.")
            self._source_status_vars[slot["key"]] = status_var
            lbl(self._sources_frame, textvariable=status_var, muted=True,
                bg=C["bg"], wraplength=310, justify="left").pack(anchor="w", pady=(0, 8))

    def _instr_load_source_slot(self, slot_key: str):
        spec = self._current_instrument()
        slot = next(s for s in spec["sources"] if s["key"] == slot_key)
        path = filedialog.askopenfilename(
            title=f"Select {slot['label']}",
            filetypes=slot["file_types"])
        if not path:
            return
        try:
            df = slot["parse"](path)
            self._instr_source_dfs[slot_key] = df
            n = len(df)
            unmapped = df.attrs.get("unmapped_keys")
            self._source_status_vars[slot_key].set(f"{Path(path).name}  ({n} coefficients)")
            self.log(f"Parsed {Path(path).name}: {n} coefficients")
            if unmapped:
                self.log(f"  (unrecognised keys passed through as-is: {unmapped})",
                          error=True)
        except Exception as e:
            self._instr_source_dfs.pop(slot_key, None)
            self._source_status_vars[slot_key].set("No file loaded.")
            messagebox.showerror("Parse error", str(e))
            self.log(f"ERROR parsing {Path(path).name}: {e}", error=True)

    def _instr_load_ci(self):
        path = filedialog.askopenfilename(
            title="Select CI calibration CSV",
            filetypes=[("CSV files", "*.csv *.CSV"), ("All files", "*.*")])
        if not path:
            return
        self._instr_ci_path = path
        self._instr_ci_var.set(Path(path).name)
        self.log(f"Loaded CI CSV: {Path(path).name}")

    def _instr_compare(self):
        spec = self._current_instrument()
        required_keys = [s["key"] for s in spec["sources"]]
        missing_slots = [s["label"] for s in spec["sources"] if s["key"] not in self._instr_source_dfs]
        if missing_slots:
            messagebox.showinfo(
                "Load all source files",
                "Still need: " + ", ".join(missing_slots))
            return
        if not self._instr_ci_path:
            messagebox.showinfo("No CI CSV", "Load a CI calibration CSV first.")
            return
        try:
            source_df = spec["combine"](self._instr_source_dfs)
            result = compare_source_to_ci(source_df, self._instr_ci_path)
        except Exception as e:
            messagebox.showerror("Comparison error", str(e))
            self.log(f"ERROR comparing: {e}", error=True)
            return

        self._instr_compare_df = result
        for item in self.instr_tree.get_children():
            self.instr_tree.delete(item)
        ...
        for _, r in result.iterrows():
            status = r["status"]
            if status == "match":
                tag, mark, note = "ok", "✓", "match"
            elif status == "mismatch":
                pct = r["delta_pct"]
                note = f"MISMATCH (Δ {pct:.4g}%)" if pct is not None else "MISMATCH"
                tag, mark = "bad", "✗"
            elif status == "missing_in_ci":
                tag, mark, note = "missing", "·", "not in CI CSV"
            else:  # missing_in_source
                tag, mark, note = "missing", "·", "not in source file(s) — verify manually"

            self.instr_tree.insert("", "end", tags=(tag,), values=(
                mark, r["serial"], r["name"],
                fmtv_compact(r["source_value"]), fmtv_compact(r["ci_value"]),
                f"{r['delta_pct']:.4g}" if pd.notna(r.get("delta_pct")) else "—",
                note,
            ))

        counts = ci_summarize(result)
        self._instr_summary_var.set(
            f"{counts.get('match', 0)} match   "
            f"{counts.get('mismatch', 0)} mismatch   "
            f"{counts.get('missing_in_ci', 0)} not in CI CSV   "
            f"{counts.get('missing_in_source', 0)} not in source file(s)"
        )
        if counts.get("mismatch"):
            self.log(f"Compare: {counts.get('mismatch')} MISMATCH found — check the "
                      "Instrument Compare tab.", error=True)
        else:
            self.log(f"Compare: all coefficients present in both match ✓")

    def _instr_export(self):
        if self._instr_compare_df.empty:
        ...
        if not path:
            return
        self._instr_compare_df.to_csv(path, index=False)
        self.log(f"Exported comparison report to {Path(path).name}")
